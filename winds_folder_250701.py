...
folder_path = r'\\Ugrid\sml_nas\연구용데이터\태풍이미지\1.카눈'
...
# for i in range(5):
    ...
    wind_speed, wind_dir = WindSpeedDir(file_path1, file_path2, file_path3, gosan_x, gosan_y)
    wind_speed_excel.append(wind_speed)
    wind_dir_excel.append(wind_dir)
    print(f"(index {i}) wind Speed : {wind_speed} [m/s], wind_dir : {wind_dir} [deg]\n")

# excel로 저장
df1 = pd.DataFrame(index, columns=['Index'])
df2 = pd.DataFrame(wind_speed_excel, columns=['Speed [m/s]'])
df3 = pd.DataFrame(wind_dir_excel, columns=['Direction [deg]'])
merged_df = pd.concat([df1, df2, df3], axis=1)
merged_df.to_excel('wind_from_satelliteImage_inGosan_카눈.xlsx', sheet_name = 'WindData')
