...
% 실제 시작 시각 : 2월 26일(수) 오전 9시 50분
clc; clear all; close all;
gpuDevice([]); % GPU 초기화
g = gpuDevice;
reset(g); % 현재 사용 중인 GPU 리셋
gpuInfo = gpuDevice();
...
trainImdsSequence = gather(trainImdsSequence);
...
load("model_checkpoint_Generator_Epoch29.mat", 'dlnetGenerator');
...
for epoch = 1:numEpochsLSTM
    ...
    while hasdata(trainImdsSequence)
        ...
        if size(data, 1) == numInputFrames + numTargetFrame
            ...
            fprintf("Epoch : %d, Iteration : %d, Training Loss : %f, Eplashed Time : %s\n",epoch, iteration, loss, string(D));
        end
    end
    if epoch ~= prev_Epoch
        filename = sprintf('model_checkpoint_1h_%d.mat', epoch);
        file_path = 'K:\code\predict2hmodel(latent64, input128)'
        if ~exist(file_path)
            mkdir(file_path)
        end
        save(fullfile(file_path, filename), 'dlnetLSTM');
        fprintf('Epoch %d: 모델 저장 완료\n', epoch);
    end
    prev_Epoch = epoch
...
save('dlnetLSTM_128ir(latentvector64, timeStep10min)_PredictTime2h', "dlnetLSTM")
...
% Get output from LSTM.
lstmNet = dlupdate(@gather, lstmNet);
generatorNet = dlupdate(@gather, generatorNet);
input = gather(input);
