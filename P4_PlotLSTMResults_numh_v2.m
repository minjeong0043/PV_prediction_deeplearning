...
load(sprintf('model_checkpoint_1h_2.mat', Epoch), 'dlnetLSTM')

numInputFrames = 3; % 3시간
numTargetFrame = 1; % 2시간
...
while i <101
    ...
    outputFolder = sprintf('C:\\Users\\Minjeong\\Desktop\\code\\predict1hmodel\\predict1h_Epoch%d', Epoch);
    if ~exist(outputFolder)
        mkdir(outputFolder);
    end
    saveas(p, fullfile(outputFolder, sprintf('Predict1_Epoch%d_%d.png', Epoch, i)));
    i = i + 1;
    close(p);
    ...
    if size(targetforPlot, 3) == 3
        ...
        targetforPlot_gray = gather(targetforPlot_gray);
    else
        targetforPlot_gray = gather(targetforPlot);
    end
    if size(generatorOutforPlot, 3) == 3
        generatorOutforPlot_gray = rgb2gray(generatorOutforPlot);
        generatorOutforPlot_gray = gather(generatorOutforPlot_gray);
    else
        generatorOutforPlot_gray = gather(generatorOutforPlot);
    end
...
T = table(gather(mseVals), gather(ssimVals), gather(ofVals), gather(corrVals), ...
    'VariableNames', {'MSE', 'SSIM', 'OF', 'Pearson Correlation'});
writetable(T, sprintf('model(predict-%dh, Epoch-%d)_Results.xlsx', predictNum, Epoch));
fprintf("엑셀 파일 저장 완.\n");
